...
export class ChatPromptTemplate extends BaseChatPromptTemplate {
    ...
    async formatMessages(values) {
        ...
        for (const promptMessage of this.promptMessages) {
            const inputValues = promptMessage.inputVariables.reduce((acc, inputVariable) => {
                // Check if the input variable is present in allValues
                if (inputVariable in allValues) {
                    // Check if the input variable is an empty array and skip if it is
                    if (Array.isArray(allValues[inputVariable]) && allValues[inputVariable].length === 0) {
                        return acc; // Skip this variable
                    }
                    acc[inputVariable] = allValues[inputVariable];
                }
                // If the variable is not present or is an empty array, it's already skipped
                return acc;
            }, {});
    ...
    async partial(values) {
        ...
        // Filter out the input variables that are provided in values
        promptDict.inputVariables = this.inputVariables.filter((iv) => !(iv in values));
    
        // Merge the existing partial variables with the new values
        promptDict.partialVariables = {
            ...(this.partialVariables ?? {}),
            ...values,
        };
        
        // Return a new instance of ChatPromptTemplate with the updated dictionary
