...
export class AssignmentCustomQAChain {
    ...
    public static fromLLM(model: OpenAIChat, index: any, namespaces: string[], options: CustomQAChainOptions): AssignmentCustomQAChain {
        return new AssignmentCustomQAChain(model, index, namespaces, options);
    }
...
private async getRelevantDocs(question, filter: any): Promise<PineconeResultItem[]> {
    ...
    public async call({ question, questionEmbed, chat_history, namespaceToFilter}: { question: string; questionEmbed: any; chat_history: ChatMessage[], namespaceToFilter: any}, ): Promise<CallResponse> {
        ...
        on class material related to Cornell classes while developing your answers to questions using the formatting instructions below. While interacting, 
